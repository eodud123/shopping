...
<script src='https://code.jquery.com/jquery-3.3.1.min.js'></script>	 
  <meta charset="utf-8">
	...
	function moveWrite(){
		location.href = "/controller/report/reportBoardWrite";
		}
	</script>
					...
					<table class="table table-hover">
						...
						<c:forEach items="${list}" var = "list">
							<tr>
								...
								<td><c:out value="${list.boardcate}" /></td>
								<td>
									<a href="/controller/report/reportBoardDetailView?report_no=${list.report_no}&page=${scri.page}&perPageNum=${scri.perPageNum}&searchType=${scri.searchType}&keyword=${scri.keyword}"><c:out value="${list.report_title}" /></a>
								</td>
								<td><c:out value="${list.member_id}" /></td>
								<td><fmt:formatDate value="${list.report_regdate}" pattern="yyyy-MM-dd"/></td>
								<td><c:out value="${list.report_hitcount}" /></td>
							</tr>
						</c:forEach>
					</table>
					...
					<div class="search row">
						<div class="col-xs-2 col-sm-2">
							<select name="searchType" class="form-control">
								...
								<option value="d"<c:out value="${scri.searchType eq 'd' ? 'selected' : ''}"/>>분류</option>
						...
						<script>
							 $(function(){
								 $('#searchBtn').click(function() {
									 self.location = "reportBoardList" + '${pageMaker.makeQuery(1)}' + "&searchType=" + $("select option:selected").val() + "&keyword=" + encodeURIComponent($('#keywordInput').val());
								 });
							 });   
						</script>
					</div>
					<div class="col-md-offset-3">
						<ul class="pagination justify-content-center">
							<c:if test="${pageMaker.prev}">
								<li class="page-item"><a href="reportBoardList${pageMaker.makeSearch(pageMaker.startPage - 1)}" class="page-link">이전</a></li>
							</c:if> 
							
							<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
								<li class="page-item" <c:out value="${pageMaker.cri.page == idx ? 'class=info' : ''}" />>
								<a href="reportBoardList${pageMaker.makeSearch(idx)}" class="page-link">${idx}</a></li>
							</c:forEach>
							
							<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
								<li class="page-item"><a href="reportBoardList${pageMaker.makeSearch(pageMaker.endPage + 1)}" class="page-link">다음</a></li>
							</c:if> 
						</ul>
					</div>
				</form>
