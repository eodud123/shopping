...
<jsp:include page="../common/header.jsp"></jsp:include>
	<div class="container">
			...
			<c:if test="${msg != 'login_error'}">
				...
				<button type="button" class="tablinks active" onclick="location.href='${pageContext.request.contextPath}/member/myWroteList'">자유게시판</button> 
				 <button type="button" class="tablinks" onclick="location.href='${pageContext.request.contextPath}/member/myReportWroteList'">신고게시판</button>
				 <button type="button" class="tablinks" onclick="location.href='${pageContext.request.contextPath}/member/myQnaWroteList'">문의게시판</button>  
				 <button type="button" class="tablinks" onclick="location.href='${pageContext.request.contextPath}/member/myASWroteList'">교환/반품/환불 게시판</button>
					...
					<div>
						 <ul>
						  <c:if test="${pageMaker1.prev}">
						   <li><a href="myWroteList?page=${pageMaker1.startPage - 1}">이전</a></li>
						  </c:if> 
						  
						  <c:forEach begin="${pageMaker1.startPage}" end="${pageMaker1.endPage}" var="idx">
						   <li><a href="myWroteList?page=${idx}">${idx}</a></li>
						  </c:forEach>
						    
						  <c:if test="${pageMaker1.next && pageMaker1.endPage > 0}">
						   <li><a href="myWroteList?page=${pageMaker1.endPage + 1}">다음</a></li>
						  </c:if> 
						 </ul>
						</div>
