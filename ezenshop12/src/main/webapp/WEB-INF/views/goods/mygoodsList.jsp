 	...
 	<div id="container_box">
 		...
 		<tbody>
		 	 <c:forEach items="${mygoodsList}" var="mygoodsList">
		 	 	<c:if test="${member.MEMBER_ID == mygoodsList.MEMBER_ID}">
			  	<tr>
				   <td><img src="${pageContext.request.contextPath}/${mygoodsList.GOODS_THUMBIMG}"></td>
				   <td>${mygoodsList.GOODS_NO}</td>
				   <td><a href="${pageContext.request.contextPath}/admin/goods/goodsdetail?n=${mygoodsList.GOODS_NO}">${mygoodsList.GOODS_NAME}</a></td>
				   <td>${mygoodsList.MEMBER_ID}</td>
				   <td>${mygoodsList.GOODS_CATEGORY}</td>
				   <td><fmt:formatNumber value="${mygoodsList.GOODS_PRICE}" pattern="###,###,###"/></td>
				   <td><fmt:formatNumber value="${mygoodsList.GOODS_DCPRICE}" pattern="###,###,###"/></td>
				   <td>${mygoodsList.GOODS_STATUS}</td>
				   <td>${mygoodsList.GOODS_DATE}</td>
			 	 </tr>
			 	  </c:if>
		 	 </c:forEach>
 		</tbody>
