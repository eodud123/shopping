		...
		<h3>내 주문 내역</h3>
			<section id="content">
				<ul class="myorderList">
					...
					<c:if test="${member.MEMBER_ID == myorderList.MEMBER_ID}">
					<li>
						<div>
							<p><span>주문번호</span><a href="${pageContext.request.contextPath}/shop/orderView?n=${myorderList.ORDER_ID}">${myorderList.ORDER_ID}</a></p>
							<p><span>수령인</span>${myorderList.ORDER_REC}</p>
							<p><span>주소</span>${myorderList.MEMBER_ADDR}</p>
							<p><span>가격</span><fmt:formatNumber pattern="###,###,###" value="${myorderList.ORDER_AMOUNT}" />원 </p>
							<p><span>상태</span>${myorderList.ORDER_DELIVERY}</p>
						</div>
