	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$(".modity_btn").on("click", function(){
				formObj.attr("action", "/controller/board/boardModify");
			...
			$(".delete_btn").on("click", function(){
				...
				formObj.attr("action", "/controller/board/boardRemove");
			...
			$(".list_btn").on("click", function(){
				...
				location.href = "/controller/board/boardList?page=${scri.page}"
			...
			$(".commentModify").on("click", function(){
				location.href = "/controller/board/commentModify?fr_no=${read.fr_no}"
								+ "&page=${scri.page}"
								+ "&perPageNum=${scri.perPageNum}"
			...
			$(".commentDelete").on("click", function(){
				location.href = "/controller/board/commentDelete?fr_no=${read.fr_no}"
					+ "&page=${scri.page}"
					+ "&perPageNum=${scri.perPageNum}"
					+ "&searchType=${scri.searchType}"
					+ "&keyword=${scri.keyword}"
		...
		<div class="container">
			...
			<section id="container">
				<form name="readForm" role="form" method="post">
					<input type="hidden" id="fr_no" name="fr_no" value="${read.fr_no}" />
				...
				<div class="form-group">
					...
					<input type="text" id="title" name="title" class="form-control" value="${read.fr_title}" readonly="readonly" />
				</div>
				<div class="form-group">
					<label for="content" class="col-sm-2 control-label">내용</label>
					<textarea id="content" name="content" class="form-control" readonly="readonly"><c:out value="${read.fr_content}" /></textarea>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<fmt:formatDate value="${read.fr_date}" pattern="yyyy-MM-dd" />	
				</div>
				<hr>
				<span>파일 목록</span>
				...
				<c:if test="${member.MEMBER_ID == read.member_id || member.MEMBER_VERIFY == 1}">	
					...
					<button type="button" class="delete_btn btn btn btn-danger btn-sm">삭제</button>
					</c:if>
				</div>
