	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$(".update_btn").on("click", function(){
				formObj.attr("action", "/controller/notice/noticeModify");
			...
			$(".delete_btn").on("click", function(){
				...
				formObj.attr("action", "/controller/notice/noticeRemove");
			...
			$(".list_btn").on("click", function(){
				...
				location.href = "/controller/notice/noticeList?page=${scri.page}"
			...
			$(".commentModify").on("click", function(){
				location.href = "/controller/notice/commentModify?notice_no=${read.notice_no}"
								+ "&page=${scri.page}"
								+ "&perPageNum=${scri.perPageNum}"
			...
			$(".commentDelete").on("click", function(){
				location.href = "/controller/notice/commentDelete?notice_no=${read.notice_no}"
					+ "&page=${scri.page}"
					+ "&perPageNum=${scri.perPageNum}"
					+ "&searchType=${scri.searchType}"
					+ "&keyword=${scri.keyword}"
		...
		function fn_fileDown(fileNo){
			...
			formObj.attr("action", "/controller/notice/fileDown");
			formObj.submit();
		}
			...
			<section id="container">
				...
				<div class="form-group" style="border: 1px solid #dbdbdb;">
					<c:forEach var="file" items="${file}">
						...
						<img src="/controller/notice/fileDown?FILE_NO=${file.FILE_NO}" width=100 height=100 onclick="OnloadImg(this.src)"/>
					</c:forEach>
					</div>
				
				<hr>
				<div>
				<button type="button" class="list_btn btn btn-outline-info btn-sm">목록</button>	
				<c:if test="${member.MEMBER_VERIFY == 1}">   
					<button type="button" class="update_btn btn btn-success btn-sm">수정</button>
